...
def recalculate_fee(fee, fromdate, todate, vehicle_tye_id):
    ...
    tout=todate
    ndayfee=0
    if (tout.date()-tin.date()).total_seconds()//3600//24>=1:
        ndayfee=(tout.date()-tin.date()).total_seconds()//3600//24 
    if (tout-tin).total_seconds()>1800:
        day_fee=20000
        night_fee=20000
        #day 1:
        m1=day_fee
        if (tin-(datetime.strptime(tin.strftime('%d/%m/%Y')+' 06:00:00', '%d/%m/%Y %H:%M:%S'))).total_seconds()<0:
            m1+=night_fee
        if (ndayfee > 0 or ((tout-(datetime.strptime(tin.strftime('%d/%m/%Y')+' 21:00:00', '%d/%m/%Y %H:%M:%S'))).total_seconds()>0)):
            m1 += night_fee
        #print((tout-(datetime.strptime(tin.strftime('%d/%m/%Y')+' 21:00:00', '%d/%m/%Y %H:%M:%S'))).total_seconds(),m1)
        #day 2...:
        m2=0
        m2=(day_fee + night_fee)*int(ndayfee-1)
        if m2<0: m2=0
        #print(m2)
        #day n:
        mn=0
        if ndayfee > 0:
            if (tout-(datetime.strptime(tout.strftime('%d/%m/%Y')+' 06:00:00', '%d/%m/%Y %H:%M:%S'))).total_seconds()>0:
                mn=day_fee
                if (tout-(datetime.strptime(tout.strftime('%d/%m/%Y')+' 21:00:00', '%d/%m/%Y %H:%M:%S'))).total_seconds()>0:
                    mn += night_fee
        #print((tout.date()-tin.date()).total_seconds()//3600//24,mn)
        fee=int(m1+m2+mn)
        #print(fee)
        if (tout-tin).total_seconds()/3600<=10 and fee>day_fee:
            fee=day_fee
        elif (tout-tin).total_seconds()/3600>10 and (tout-tin).total_seconds()/3600<=24 and fee>day_fee+night_fee:
            fee=day_fee+night_fee
    return fee
