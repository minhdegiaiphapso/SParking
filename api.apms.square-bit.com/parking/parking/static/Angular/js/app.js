    ...
    myapp.controller("activesamples", function($scope,$location, $anchorScroll,dataService) {
        ...
        $scope.callfee=function(){
            ...
            //alert(expired)
            dataService.getcallfee(id,fromdate,todate,expired,notregis).then(function(data){
        ...
        $scope.post_sampleregis=function(item){
            ...
            var tmp={"activedate":$scope.convertdate(item.activedate),
                ...
                "sampleid2":item.samplesset2.sampleselected.id};
            //console.log(tmp);
            var obj=JSON.stringify(tmp);
            //console.log(obj);
...
//            $anchorScroll();
        ...
        $scope.calculate=function(it,item)
        {
             ...
             $scope.demo.samplename2=it.samplename2;
             $scope.demo.id=it.id;
             $scope.demo.expireddate=null;
             $scope.demo.detail=[]
        }
        $scope.filterformular=function(it)
        {
            console.log(it);
            var filterstr=it.fiterformula;
            var formulars=it;
            var l=formulars.length;
            for(i=0;i<l;i++)
            {
                if(formulars[i].callname.toLowerCase().includes(filterstr.toLowerCase()))
                    formulars[i].canshow=true;
                else
                    formulars[i].canshow=false;
            }
            console.log(it);
        }
    });
    myapp.controller('NNdetail',function($scope,dataService)
    {
        $scope.getdetail=function(id){
            $scope.loading=true;
             dataService.getSampleFeeDetailNN(id).then(function(data){
    ...
    myapp.controller('myCtrl', function($scope,dataService) {
       ...
       dataService.getSampleFees(0,0).then(function(data){
        ...
        $scope.addsample.postsamplefees=function(){
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if($scope.addsample.typemethodselected.id==3)
                    {
                        obj.type=5;
                        data.freetime=$scope.addsample.freetime;
                        data.callname=$scope.addsample.feename;
                        data.startdate=$scope.addsample.NN.daystart;
                        data.startnight=$scope.addsample.NN.nightstart;
                        data.dayformula=$scope.addsample.NN.formulas.dayselected.id;
                        data.nightformula=$scope.addsample.NN.formulas.nightselected.id;
                        data.optioncase=$scope.addsample.NN.OptionCaseSelected.id;
                        data.fee24=$scope.addsample.NN.Fee24h;
                        data.maxfee=$scope.addsample.NN.MaxFee;
                        obj.data=data;
                    }
                    else
                    {
        ...
        { id: 3, name: 'Mở rộng nghị định 2018',issettotal:false }
        ];
        ...
        $scope.addsample.NN=[];
         dataService.getFormulaFees().then(function(data){
            //$scope.addsample.NN.formulas=data.data;
            var data=data.data;
            $scope.addsample.NN.formulas=[];
            angular.forEach(data, function(value, key) {
                if(value.feetype==2)
                    $scope.addsample.NN.formulas.push(value);
            });
            $scope.addsample.NN.formulas.nightcolor='orange';
            $scope.addsample.NN.formulas.daycolor='orange';
        });

        $scope.addsample.NN.OptionCase=[
            { id: 1, name: 'Chọn ca ngày hay đêm theo mốc vào thuộc ngày hay đêm', value:"Trường hợp 1" },
            { id: 2, name: 'Chọn ca ngày hay đêm theo phần thời gian thuộc ngày hay đêm nhiều hơn', value:"Trường hợp 2" },
            { id: 3, name: 'Nếu phần thời gian ngày đêm bằng nhau chọn theo trường hợp 1', value:"Trường hợp 3" },
            { id: 4, name: 'Nếu phần thời gian ngày đêm bằng nhau chọn theo ca ngày', value:"Trường hợp 4" },
            { id: 5, name: 'Nếu phần thời gian ngày đêm bằng nhau chọn theo ca đêm', value:"Trường hợp 5" },
        ];
        $scope.addsample.NN.OptionCaseSelected=$scope.addsample.NN.OptionCase[0];
    ...
    myapp.controller('FormFee',function($scope,dataService){
         ...
         $scope.copyitem=function(item)
        {
            $scope.loading=true;
            obj=[];
            obj.data=JSON.stringify(item);
            //console.log(item);
            dataService.copyFormFee(obj).then(function(data)
                {
                    if(data.data!="fail" && data.data!="failname")
                    {
                          $scope.formulas.splice( 0, 0, data.data )
    ...
    myapp.controller('FormBill',function($scope,dataService){
        ...
        $scope.copyitem=function(item)
        {
            ...
            obj.data=JSON.stringify(item);
            //console.log(item);
            dataService.copyFormBill(obj).then(function(data)
                {
                    if(data.data!="fail" && data.data!="failname")
                    {
                          $scope.formulas.splice( 0, 0, data.data )
    ...
    myapp.directive('sampleNN', function() {
        //console.log(dataService);
        var directive = {};
        directive.restrict = 'E';
        directive.templateUrl =static_url + 'Angular/TemplateHtml/SampleNNDetail.html';
        return directive;
    });
    ...
    myapp.service('dataService', function($http) {
        ...
        this.getcallfee = function(id,from,to,expired, notregis) {
            return $http({
                method: 'GET',
                url: url_root+'/get/callfeeactive/'+id+'/'+from+'/'+to+'/'+expired+'/'+notregis,
                dataType: 'json',
                contentType: "application/json"
             });
        };
        ...
        //Lấy chi tiết sample fee loại định mức nhà nước mở rộng 2018Dec
        this.getSampleFeeDetailNN = function(feeid) {
            return $http({
                method: 'GET',
                url: url_root+'/get/detailfeeNN/'+feeid,
                dataType: 'json',
                contentType: "application/json"
             });
        };
        ...
        //copyFormFee
        this.copyFormFee=function(obj)
        {
            var form=new FormData();
            form.append("data",obj.data);
            return $http({
                method: 'POST',
                url: url_root+'/post/copyfromfee/',
                data:form,
                dataType: 'json',
                contentType: "application/json",
                headers: { 'Content-Type': undefined},
        ...
        //copyFormBill
        this.copyFormBill=function(obj)
        {
            var form=new FormData();
            form.append("data",obj.data);
            return $http({
                method: 'POST',
                url: url_root+'/post/copyfrombill/',
                data:form,
                dataType: 'json',
                contentType: "application/json",
                headers: { 'Content-Type': undefined},
        ...
        this.postSampleFees = function(obj){
            ...
            switch(type)
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    form.append("feetype","country")
                    form.append("callname",data.callname);
                    form.append("fee24",data.fee24);
                    form.append("maxfee",data.maxfee);
                    form.append("startdate",data.startdate);
                    form.append("startnight",data.startnight);
                    form.append("formuladay",data.dayformula);
                    form.append("formulanight",data.nightformula);
                    form.append("optioncase",data.optioncase);
                    form.append("freetime",data.freetime);
                    break;
            }
            return $http({
